...
assert(reloaded:add(3, string.rep("b", 64)))
assert(reloaded:save_atomic())
local ordered = assert(delivery.Ledger.new(ledger_path, fs)):ordered_matching({
  ["7"] = string.rep("a", 64), ["3"] = string.rep("b", 64)
})
t.eq("ledger persists delivery order across slot reuse", ordered[1].index .. "," .. ordered[2].index, "7,3")
assert(reloaded:remove(7, string.rep("a", 64)))
assert(reloaded:remove(3, string.rep("b", 64)))
assert(reloaded:save_atomic())
